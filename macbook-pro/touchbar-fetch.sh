...
# 安全网：脚本结束时必须产生输出，否则 MTMR 会显示 "error"
HAD_OUTPUT=false
on_exit() {
  if [ "$HAD_OUTPUT" = false ]; then
    echo "\033[90m⚡ 暂时无法连接\033[0m"
  fi
}
trap on_exit EXIT

mark_output() { HAD_OUTPUT=true; }
...
export FIVE_PCT FIVE_RESET SEVEN_PCT SEVEN_RESET SONNET_PCT SONNET_RESET UPDATED TRANSPORT
